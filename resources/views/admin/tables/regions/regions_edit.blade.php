    ...
    @isset($region)
        <form id="regions_update" action="{{ route('admin.regions.update', ['id' => $region->id]) }}" method="POST" novalidate>
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    <input type="text" name="name"  class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" id="name" value="{{ old('name' ,$region->name) }}">
                    @if($errors->has('name'))
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-10">
                    <input type="text"  name="slug" class="form-control" id="slug" value="{{ old('slug', $region->slug) }}">
                    @if($errors->has('slug'))
                        <span class="invalid-feedback" >
                            {{ $errors->first('slug') }}
                        </span>
